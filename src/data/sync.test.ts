...
import type { Child, Treatment, DiaperEntry, Entry, Measurement } from '@/types/models';
...
describe('uploadUnsynced', () => {
  it('uploads children before entries, measurements and treatments', async () => {
    const log: string[] = [];
    const state: UploadState = {
      children: [child({ id: 'c1' })],
      entries: [diaperEntry({ id: 'e1', childId: 'c1' })],
      measurements: [measurement({ id: 'm1', childId: 'c1' })],
      treatments: [treatment({ id: 'cu1', childId: 'c1' })],
    };
    const deps: UploadDeps = {
      ...
      pushTreatment: vi.fn(async (c) => {
        log.push(`treatment:${c.id}`);
        return 40;
      }),
    };
    ...
    expect(log).toEqual(['child:c1', 'entry:e1', 'measurement:m1', 'treatment:cu1']);
  });
  ...
  it('passes the newly-assigned server child id alongside the treatment (childId stays local)', async () => {
    const state: UploadState = {
      children: [child({ id: 'child1', serverId: undefined })],
      entries: [],
      measurements: [],
    ...
    expect(pushTreatment).toHaveBeenCalledWith(expect.objectContaining({ id: 'cu1', childId: 'child1' }), 42);
  });

  it('does not push a treatment that already has a serverId', async () => {
    const pushTreatment = vi.fn(async () => 999);
    const state: UploadState = {
      ...emptyState(),
      children: [child({ id: 'c1', serverId: 5 })],
      treatments: [treatment({ id: 'cu1', childId: 'c1', serverId: 7 })],
    };

    const result = await uploadUnsynced(state, makeDeps({ pushTreatment }));

    expect(pushTreatment).not.toHaveBeenCalled();
    expect(result.treatments?.[0].serverId).toBe(7);
  });

  it('skips a treatment whose child never reached the server, without aborting the rest', async () => {
    const pushTreatment = vi.fn(async () => 400);
    const state: UploadState = {
      children: [child({ id: 'c1', serverId: undefined })],
      entries: [],
      measurements: [],
      treatments: [treatment({ id: 'cu1', childId: 'c1' }), treatment({ id: 'cu2', childId: 'c-unknown' })],
    };

    const result = await uploadUnsynced(state, makeDeps({ pushChild: vi.fn(async () => undefined), pushTreatment }));

    expect(pushTreatment).not.toHaveBeenCalled();
    expect(result.treatments?.every((c) => c.serverId == null)).toBe(true);
  });

  it('leaves a treatment unsynced when pushTreatment throws, and does not abort the rest', async () => {
    const state: UploadState = {
      children: [child({ id: 'c1', serverId: 1 })],
      entries: [],
      measurements: [],
      treatments: [treatment({ id: 'cu1', childId: 'c1' }), treatment({ id: 'cu2', childId: 'c1' })],
    };
    const pushTreatment = vi.fn(async (c: Treatment) => {
    ...
    expect(result.treatments?.find((c) => c.id === 'cu1')?.serverId).toBeUndefined();
    expect(result.treatments?.find((c) => c.id === 'cu2')?.serverId).toBe(41);
  });

  it('treats an absent `treatments` as none to push (callers pass only their slice)', async () => {
    const pushTreatment = vi.fn(async () => 400);
