...
/**
 * One-line summary of a running timer, reading as its future entry would so a timer
 * row and the entry it becomes say the same thing, minus anything that needs an end.
 * Every field but `start` is optional on a `Timer` (they are only filled in once the
 * running timer is edited), so an unset one says nothing rather than guessing.
 */
function timerDetail(tm: Timer): string {
  ...
  switch (tm.saveAs) {
    case 'feeding':
      ...
      // Same dual-purpose `amount` as a feeding entry: millilitres from a bottle, a
      // dimensionless intake level at the breast.
      if (tm.amount)
...
export function detailFor(item: TimelineItem): string {
  // Timers are split off rather than widening the switch below, which keeps that an
  // exhaustive match over `Entry`: a newly added activity becomes a compile error
  ...
  switch (e.type) {
    case 'feeding':
      return [
        ...
        // A breast feed's `amount` is a dimensionless intake level, not millilitres,
        // so it is shown as its word ("A little"). Formatting it as a measurement
        // would attach a false unit (a level 2 would read as "0.1 fl oz").
    ...
    case 'medication': {
      // The unit is Baby Buddy's free-text `dosage_unit`, NOT a units.ts quantity, so
