...
describe('allowsMultipleChildren gates the "log for both" affordance', () => {
  ...
  it('refuses pumping, so duplicating a session cannot double-count the milk', () => {
    // Pumping is parent-side: one session produces one volume, and copying it onto
    // each child inflates every aggregate built on it.
...
describe('three-level scales', () => {
  ...
  for (const [name, set] of sets) {
    ...
    // The invariant every bucket has to hold: what the scale writes, it reads back.
    // Without it a level saved today would light up a different button tomorrow.
  ...
  it('intake reads a legacy score above 3 by thirds, since it cannot be a level', () => {
    // Nothing writes above 3, so 4 and up can only be an old 1-to-10 score and gets
    // the same reading the wire format gives one.
  ...
  it('intake is deliberately non-monotonic across the 3/4 boundary', () => {
    ...
    // monotonic bucket: that would either mangle today's levels or misread every old
    // score. 3 is the top LEVEL, 4 is a mid old SCORE.
  ...
  it('intake keeps the residual ambiguity to old scores of 2 and 3', () => {
    // A stored 2 or 3 is both a valid level and a valid old score, so it reads as
    // today's level. Every other old score reads the same either way, which is what
    // keeps the two directions of sync agreeing.
